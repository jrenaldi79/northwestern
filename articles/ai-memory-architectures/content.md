...
Today's chat interfaces and coding tools have landed on the same workaround: compress what came before. ChatGPT and Claude summarize older turns in a conversation to keep the active context within budget. Coding harnesses like Claude Code, Cursor, and Copilot do the same thing with the accumulated history of user prompts, tool calls (file reads, terminal commands, search results), and AI responses - once the context fills up, the system condenses earlier exchanges into a summary and continues from there. The problem is that summarization is lossy by definition. Every compression pass drops details that seemed minor at the time but turn out to matter later: the specific flag a user set in a config file, the reason a particular design approach was rejected, the edge case someone mentioned offhand three hours into a session. What you end up with is swiss cheese memory - the system retains the general shape of what happened but has holes where specifics used to be. A coding assistant might remember that you refactored the authentication module but not that you deliberately kept the legacy fallback path for on-prem customers. A wealth advisor's AI copilot might recall that a client wants aggressive growth but forget that the client specifically said to avoid fossil fuel holdings after a conversation about their foundation's climate mandate. The compression is invisible to the user, who reasonably assumes the system still knows everything they told it. It does not.
